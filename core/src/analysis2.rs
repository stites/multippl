...
pub struct AnalysisEnv {
    seen: HashSet<Var>,
    above_below: HashMap<SamplingPosition, (HashSet<Var>, HashSet<Var>)>,
...
impl AnalysisEnv {
    ...
    pub fn insert_below(&mut self, v: &Var) {
        for (_, (_, b)) in self.above_below.iter_mut() {
            b.insert(v.clone());
        }
    }

    pub fn insert_above(&mut self, p: SamplingPosition, v: &Var) {
        ...
        match self.above_below.get(&p) {
            Some((a, b)) => {
                self.above_below.insert(p, (above, b.clone()));
            }
            None => {
                self.above_below.insert(p, (above, HashSet::new()));
            }
        }
        let (a, b) = self.above_below.get(&p).unwrap();
    ...
    pub fn analyze_anf(&mut self, a: &AnfAnn) -> Result<(), CompileError> {
        ...
        match a {
            AVar(v, s) => {
                use SamplingContext::*;
                use SamplingPosition::*;
                let key = match &self.sampling_context {
                    Unset => panic!("impossible"),
                    Set(v) => Pos(v.id(), self.tuple_ix),
                    Sampling(v) => Pos(v.id(), self.tuple_ix),
                    SamplingWithLet(v, _) => Pos(v.id(), self.tuple_ix),
                };
                match self.state {
                    State::Declaration => panic!("impossible"),
                    State::Dependence => {
                        self.insert_above(key, v);
                        self.insert_below(v);
                    }
                    State::Projection => {}
                }
