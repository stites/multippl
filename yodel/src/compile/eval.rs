...
pub fn ite_bdds(state: &mut super::eval::State, guard : BddPtr, l : &EVal, r : &EVal) -> EVal {
    use EVal::*;
    match (l, r) {
        (EBdd(tdist), EBdd(fdist)) => {
            EBdd(state.mgr.ite(guard, *tdist, *fdist))
        }
        (EProd(ts), EProd(fs)) => {
            let (l, r) = if ts.len() < fs.len() {
                 let diff = fs.len() - ts.len();
                 ([ts.clone(), vec![EBdd(BddPtr::PtrFalse); diff]].concat(), fs.clone())

            } else if ts.len() < fs.len() {
                 let diff = ts.len() - fs.len();
                 (ts.clone(), [fs.clone(), vec![EBdd(BddPtr::PtrFalse); diff]].concat())
            } else {
                (ts.clone(), fs.clone())
            };
