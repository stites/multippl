...
def network(suffix=""):
    ...
    return (n40l, n41l, n42l, n43l, n44l, n45l, n46l, n47l)

sites = ["npackets"]
truth = [3.0 * (
  2 * 0           - 2 * (1 - 0)
+ 2 * 0.5         - 2 * (1 - 0.5)
+ 2 * 0.246031746 - 2 * (1 - 0.246031746)
+ 2 * 0.253968254 - 2 * (1 - 0.253968254)
+ 2 * 0.119047619 - 2 * (1 - 0.119047619)
+ 2 * 0.380952381 - 2 * (1 - 0.380952381)
+ 2 * 0.130952381 - 2 * (1 - 0.130952381)
+ 2 * 0.369047619 - 2 * (1 - 0.369047619)
)]
print(truth)
...
def model():
    ...
    for ix in pyro.plate("packet", int(npackets.item())+1):
        netfwd = pyro.condition(network, data={f"n30l_{ix}": torch.zeros(1)})
