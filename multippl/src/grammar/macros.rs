...
macro_rules! val {
    ( $x:ident ) => {
        anf!($crate::grammar::Anf::<$crate::typeinf::grammar::Inferable, EVal>::AVal((), $crate::grammar::EVal::EBdd($crate::BddPtr::from_bool($x))))
    };
    (~ $x:ident ) => {
        anf!(~ $crate::grammar::Anf::<$crate::typeinf::grammar::Inferable, SVal>::AVal((), $crate::grammar::SVal::SBool($x)))
    };
    ( $y:literal, $( $x:literal ),+ ) => {{
        let mut fin = Box::new($crate::grammar::EVal::EBdd($crate::BddPtr::from_bool($y)));
        $(
            fin = Box::new($crate::grammar::Anf::EProd(fin, Box::new($crate::grammar::EVal::EBdd($crate::BddPtr::from_bool($x)))));
        )+
