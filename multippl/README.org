...
The MultiPPL compiler aims to be a nanopass-style compiler leveraging
[[file:../ttg/README.org][trees that grow]] to abstract away IR boilerplate. The
MultiPPL host language uses dynamic safety, with two initial compiler passes
which stub out a future bidirectional type system. The compiler passes are as
follows:
...
2. Type-inference: [[file:src/typeinf.rs][typeinf.rs]]. Stub for a bidirectional type inference.
3. Type-checking: [[file:src/typecheck.rs][typecheck.rs]]. Stub for a bidirectional type inference.
